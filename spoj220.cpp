...
int n, m;
char str[107][10007], s[MAX_N];
int sumLen[107], d[MAX_N];
int f[MAX_N];

bool check(int len) {
	int mn[107] = {0}, mx[107] = {0};
	int i = 1;
	...
	while (i <= m) {
		...
		memset(mn, 0x3f, sizeof mn);
		memset(mx, 0, sizeof mx);
		mn[f[sa[i - 1]]] = sa[i - 1], mx[f[sa[i - 1]]] = sa[i - 1];
		while (i <= m && height[i] >= len) {
			mn[f[sa[i]]] = min(mn[f[sa[i]]], sa[i]);
			mx[f[sa[i]]] = max(mx[f[sa[i]]], sa[i]);
			int cnt = 0;
			for (int j = 1; j <= n; ++j) 
				if (mx[j] - mn[j] >= len) ++cnt;
